...
The sample data for testing the `fetchVegdata()` functionality are queried into the local database, accepted, and queried into selected set using NSSC_Pangaea query `VEG - Veg Plots by User Site ID and Vegplot ID` targeting Site and Vegetation Plot objects, and SSRO_Alaska `Pedon/Site/Vegetation Plot by Veg Plot ID` targeting Site and Pedon objects, setting parameter for User Site or Vegplot ID  matching`"2010WY629%"`. This gives a result with `137` Site, `11` Pedon, and `139` Vegetation Plot objects.
